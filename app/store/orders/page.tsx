...
export default function StoreOrders() {
    ...
    const fetchOrders = async () => {
       try {
        ...
        const { data } = await axios.get<{ orders: StoreOrder[] }>('/api/store/orders', {headers: { Authorization: `Bearer ${token}` }})
    ...
    const openModal = (order: StoreOrder) => {
                ...
                <div onClick={closeModal} className="fixed inset-0 flex items-center justify-center bg-black/50 text-slate-700 text-sm backdrop-blur-xs z-50" >
                    <div onClick={e => e.stopPropagation()} className="bg-white rounded-lg shadow-lg max-w-2xl w-full p-6 relative">
                        ...
                        <div className="mb-4">
                            ...
                            <p><span className="text-green-700">Address:</span> {`${selectedOrder.address?.street}, ${selectedOrder.address?.city}, ${selectedOrder.address?.state}, ${selectedOrder.address?.pincode}, ${selectedOrder.address?.country}`}</p>
                        </div>
                        ...
                        <div className="mb-4">
                            ...
                            <div className="space-y-2">
                                {selectedOrder.orderItems.map((item, i) => (
                                    <div key={`${selectedOrder.id}-${i}`} className="flex items-center gap-4 border border-slate-100 shadow rounded p-2">
                                        <img
