...
const OrderSummary = ({ totalPrice, items }) => {
    ...
    const isPlusMember = has?.({ plan: 'plus' }) ?? false
    ...
    const handleCouponCode = async (event: any) => {
        ...
        <div className='w-full max-w-lg lg:max-w-[340px] bg-slate-50/30 border border-slate-200 text-slate-500 text-sm rounded-xl p-7'>
            ...
            <div className='my-4 py-4 border-y border-slate-200 text-slate-400'>
                <p>Address</p>
                {
                    selectedAddress ? (
                        <div className='flex gap-2 items-center'>
                            <p>{selectedAddress.name}, {selectedAddress.city}, {selectedAddress.state}, {selectedAddress.pincode}</p>
                                    ...
                                    <select className='border border-slate-400 p-2 w-full my-3 outline-none rounded' onChange={(e) => setSelectedAddress(addressList[e.target.value])} >
                                        <option value="">Select Address</option>
                                        {
                                            addressList.map((address, index) => (
                                                <option key={index} value={index}>{address.name}, {address.city}, {address.state}, {address.pincode}</option>
                                            ))
                                        }
                                    </select>
                                )
            ...
            <div className='pb-4 border-b border-slate-200'>
                <div className='flex justify-between'>
                    ...
                    <div className='flex flex-col gap-1 font-medium text-right'>
                        ...
                        <p>{isPlusMember ? 'Free' : `${currency}5`}</p>
